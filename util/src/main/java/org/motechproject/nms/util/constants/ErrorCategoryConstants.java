package org.motechproject.nms.util.constants;

/**
 * This class defines the common error categories used by all the modules.
 * These categories briefly indicates the error in bulk upload record.
 */
public final class ErrorCategoryConstants {

    public static final String MANDATORY_PARAMETER_MISSING = "Mandatory Parameter Missing";
    public static final String INCONSISTENT_DATA = "Inconsistent Data";
    public static final String INVALID_DATA = "Invalid Data";

    private ErrorCategoryConstants() {

    }
}
